...
Always Yes 默认不会在启动时反复弹出辅助功能授权提示。点状态栏的 ✋ 图标，菜单里会有「辅助功能：未授权（点此申请）」选项；也可以在「控制面板 → 通用」里点击「申请辅助功能权限」。如果你希望应用定时提示授权，可以在控制面板的「通用」页打开「自动提示辅助功能授权」。
...
| `⇧⌥Y` | 输入 `y` 后按回车 | `Run this command? (y/N)` 这类确认 |
